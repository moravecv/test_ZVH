...
Vypočítejte **průtok [m$^{3}\cdot$ s$^{-1}$]** v otevřeném pravidelném lichoběžníkovém korytě ve výšce $h$ = `r PR5$h` [m]. Využijte Chézyho rovnice.    
...
Dále je dán `r PR5$I` [\%] podélný sklon koryta, drsnost obou břehů $n$ = `r PR5$n`, šířka dna $b$ = `r PR5$b` [m] a sklon  
svahu $m$ = 1,5.
...
```
**Objem odtoku** je $Vs$ = `r as.integer(PR1_res$Vo)` m$^3$.   
**Objem srážky** je $Vo$ = `r as.integer(PR1_res$Vs)` m$^3$.   
