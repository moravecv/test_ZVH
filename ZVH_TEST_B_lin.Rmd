---
title: "ZVH Zápočtový TEST - Varianta `r params$varianta`"
# subtitle: "Varianta `r params$varianta`"
author: "Jméno a příjmení:"
header-includes: 
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhf{}
- \chead{Jméno a příjmení:}
- \lhead{Varianta `r params$varianta`}
- \usepackage{float}
- \usepackage{graphics}
- \usepackage{graphicx}
- \usepackage{placeins}
- \usepackage{amsmath}
date: "`r format(Sys.time(), '%d %B %Y')`"
output: pdf_document
params:
  n: 1
  d: !r Sys.Date()
  varianta: B
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(echo = TRUE)
options(OutDec = ",")
```

## Slepá mapa (15 bodů)
```{r, echo = FALSE}
vsechny_reky <- c("Kamenice",
          "Smědá",
          "Lužická Nisa",
          "Ploučnice",
          "Kamenice",
          "Labe",
          "Bílina", 
          "Chomutovka", 
          "Ohře",
          "Liboc",
          "Úpa", 
          "Metuje", 
          "Jizera", 
          "Mrlina", 
          "Cidlina", 
          "Bystřice",
          "Orlice",
          "Tichá Orlice",
          "Teplá",
          "Střela",
          "Rakovnický potok",
          "Loděnice",
          "Litavka", 
          "Berounka",
          "Vltava",
          "Mže",
          "Rokytka",
          "Výrovka", 
          "Želivka", 
          "Radbůza",
          "Úhlava",
          "Úslava",
          "Lomnice",
          "Otava", 
          "Křemelná",
          "Volyňka",
          "Blanice",
          "Doubrava",
          "Doubrava",
          "Loučná",
          "Chrudimka",
          "Moravská Sázava",
          "Třebůvka",
          "Morava",
          "Sázava",
          "Malše",
          "Lužnice",
          "Nežárka",
          "Moravská Dyje",
          "Rokytná",
          "Jihlava",
          "Oslava",
          "Dyje",
          "Svratka",
          "Svratka",
          "Svitava",
          "Osoblaha",
          "Bělá",
          "Opava",
          "Moravice",
          "Odra",
          "Olše",
          "Haná",
          "Bečva",
          "Ostravice",
          "Rožnovská Bečva",
          "Vsetínská Bečva",
          "Olšava",
          "Vlára", 
          "Morava")
vsechny_nadrze <- c("Jirkov", 
                    "Fláje",
                    "Josefův Důl",
                    "Labská",
                    "Rozkoš",
                    "Nechranice",
                    "Máchovo jezero",
                    "Žlutice",
                    "Hracholusky",
                    "Nýrsko",
                    "Vrané",
                    "Štěchovice",
                    "Slapy",
                    "Kamýk",
                    "Orlík",
                    "Kořensko",
                    "Hněvkovice",
                    "Lipno I.",
                    "Lipno II.",
                    "Bezdrev",
                    "Římov",
                    "Svět",
                    "Rožmberk",
                    "Staňkovský rybník/rybník Hejtman",
                    "Želivka - Švihov",
                    "Pastviny",
                    "Slezská Harta",
                    "Kružberk",
                    "Šance",
                    "Seč",
                    "Vír",
                    "Brněnská přehrada",
                    "Vranov",
                    "Nové Mlýny")
vybrane_reky <- sample(vsechny_reky, 10)
vybrane_nadrze <- sample(vsechny_nadrze, 5)
```


Zaneste do slepé mapy následující řeky: \par 
*`r vybrane_reky`.*
\par 
Dále v mapě vyznačte tyto nádrže: \par 
*`r vybrane_nadrze`*
\begin{figure}[h!]
  \makebox[\textwidth]{\includegraphics[width = \paperwidth]{slepa_mapa_cb.pdf}}
\end{figure}
\FloatBarrier

## Příklad 1 (7 bodů)
```{r echo = FALSE}
PR1 <- list()
PR1$A <- round(runif(1, 4, 14), 0)
PR1$Hs <- round(runif(1, 400, 900), 0)
PR1$Qa <- round(runif(1, 40, 90), 0)
```

Výpočítejte odtokový součinitel $\varphi$ v povodí o velikosti $A$ = `r PR1$A` km$^{2}$, kde roční úhrn srážek je $H_S$ = `r PR1$Hs` mm
a průměrný roční průtok v uzávěrovém profilu povodí je $Q_a$ = `r PR1$Qa` l$\cdot$ s$^{-1}$
\vskip5cm

## Příklad 2 (7 bodů)
```{r, echo = FALSE}
PR2 <- list()
PR2$A <- round(runif(1, 4, 25), 0)
PR2$Hs <- round(runif(1, 400, 1400), 0)
PR2$phi <- round(runif(1, 0, 1), 2)
```

<!-- `r if(params$varianta == "A"){"##Příklad 2 \nVypočítejte měsíční bilanci \n\n$ $\n"}` -->
Vypočtěte odtok (l$\cdot$s$^{-1}$) z území o rozloze $A$ = `r PR2$A` km$^2$, kterému náleží roční úhrn srážek $Hs$ = `r PR2$Hs` mm a odtokový součintel $\varphi$ = `r PR2$phi`.
\vskip5cm

## Příklad 3 (7 bodů)
```{r, echo = FALSE}
# Generování zadání rovnice kontinuity
PR3 <- list()
PR3$Q1 <- round(runif(1, min = 0.1, max = 10), 2) # průměr v cm2
PR3$v1 <- round(runif(1, min = 0.5, max = 4), 2)
PR3$D1 <- round(runif(1, min = 0.5, max = 4), 2)

PR3$Q2 <- round(runif(1, min = 0.1, max = 10), 2) # průměr v cm2
PR3$v2 <- round(runif(1, min = 0.5, max = 4), 2)
PR3$D2 <- round(runif(1, min = 0.5, max = 4), 2)

PR3$Q3 <- round(runif(1, min = 0.1, max = 10), 2) # průměr v cm2
PR3$v3 <- round(runif(1, min = 0.5, max = 4), 2)
PR3$D3 <- round(runif(1, min = 0.5, max = 4), 2)

PR3$Q4 <- round(runif(1, min = 0.1, max = 10), 2) # průměr v cm2
PR3$v4 <- round(runif(1, min = 0.5, max = 4), 2)
PR3$D4 <- round(runif(1, min = 0.5, max = 4), 2)
```

Potrubím protéká voda. Dopočítejte hodnoty ve čtyřech měřících profilech. Uvažujte ideální kapalinu a kruhový přůřez potrubí. Užijte znalosti, že Q$_1$ = Q$_2$.  


\begin{table}[h!]
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
Profil                 & 1          & 2          & 3          & 4            \\ \hline
v (m$\cdot$ s $^{-1}$) & `r PR3$v1` &            & `r PR3$v3` &              \\ \hline
Q (l$\cdot$ s $^{-1}$) &            &            & 2Q$_{2}$   & Q$_2$/4      \\ \hline
D (cm)                 & `r PR3$D1` & `r PR3$D2` &            &              \\ \hline
S (cm$^{2}$)           &            &            &            & 1/3 S$_3$    \\ \hline
\end{tabular}
\end{table}


\vskip10cm    

## Příklad 4 (7 bodů)
```{r, echo = FALSE}
PR4 <- list()
PR4$Q1 <- round(runif(1, 40, 90), 0)
PR4$c1 <- round(runif(1, 0.001, 0.010), 3)
PR4$Q2 <- round(runif(1, 1, 9), 0)
PR4$c2 <- round(runif(1, 0.010, 0.050), 3)
```

V pozorovaném místě byl v řece naměřen průtok $Q_1$ = `r PR4$Q1` l$\cdot$ s$^{-1}$ a zjištěna koncentrace kontaminační látky $c_1=$ `r PR4$c1`  mg$\cdot$ l$^{-1}$. Dále po toku do něj vstupuje výpust $Q_2$ = `r PR4$Q2` l$\cdot$ s$^{-1}$ s koncentrací $Q_2=$ `r PR4$c2` mg$\cdot$ l$^{-1}$. Vypočítejte celkovou koncetraci látky za soutokem.
\vskip5cm

## Příklad 5 (7 bodů)

```{r, echo = FALSE}
PR5 <- list()
PR5$h <- round(runif(1, 1, 2), 1)
PR5$n <- round(runif(1, 0.02, 0.05), 2)
PR5$b <- round(runif(1,0.5,1.5),1)
```

Vypočítejte průtok v otevřeném pravidelném lichoběžníkovém korytě ve výšce $h$ = `r PR5$h`. Využijte Chézyho rovnice.    
Vzorce pro geometrii koryta:   
\begin{align}
O &= b + 2\sqrt{(hm)^2 + h^2}\\   
S &= h(b + hm)
\end{align}

Dále je dán 2\% podélný sklon koryta, drsnost obou břehů $n$ = `r PR5$n`, šířka dna $b$ = `r PR5$b` a sklon svahu $m$ = 1,5.


\newpage
## Výsledky
#### Příklad 1
```{r, echo = FALSE}
PR1_res <- list()
PR1_res$phi <- (PR1$Qa * 1e-03 * 60 * 60 * 24 * 365) / (PR1$A * 1e+06 * PR1$Hs * 1e-03) 
```
Odtokový součinitel je $\varphi$ = `r round(PR1_res$phi, 2)`.

#### Příklad 2   
```{r, echo = FALSE}
PR2_res <- list()
PR2_res$Qout <- (PR2$Hs * 1e-03 * 60 * 60 * 24 * 365) / (PR1$A * 1e+06 * PR1$Hs * 1e-03) 
```
Odtok z povodí je $Q$ = `r round(PR2_res$Qout, 2)` $m^{3}$

#### Příklad 3
```{r, echo = FALSE}
PR3_res <- list()
PR3_res$S1 <- round((pi * PR3$D1^2)/4, 2)
PR3_res$Q1 <- round(PR3$v1 * PR3_res$S1, 2)
PR3_res$Q2 <- round(PR3_res$Q1, 2)
PR3_res$S2 <- round((pi * PR3$D2^2)/4, 2)
PR3_res$v2 <- round(PR3_res$Q2 / PR3_res$S2, 2)
PR3_res$S3 <- round(PR3_res$Q2 * 2 / PR3$v3, 2)
PR3_res$D3 <- round(sqrt((4 * PR3_res$S3) / pi), 2)
PR3_res$Q4 <- round(PR3_res$Q2 / 4, 2)
PR3_res$S4 <- round(PR3_res$S3 / 3, 2)
PR3_res$v4 <- round(PR3_res$Q4 / PR3_res$S4, 2)
PR3_res$D4 <- round(sqrt((4 * PR3_res$S4) / pi), 2)
```


\begin{table}[h!]
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
Profil                 & 1              & 2               & 3               & 4               \\ \hline
v (m$\cdot$ s $^{-1}$) & `r PR3$v1`     & `r PR3_res$v2`  & `r PR3$v3`      & `r PR3_res$v4`  \\ \hline
Q (l$\cdot$ s $^{-1}$) & `r PR3_res$Q1` & `r PR3_res$Q2`  & 2Q$_{2}$        & Q$_2$/4         \\ \hline
D (cm)                 & `r PR3$D1`     & `r PR3$D2`      & `r PR3_res$D3`  & `r PR3_res$D4`  \\ \hline
S (cm$^{2}$)           & `r PR3_res$S1` & `r PR3_res$S2`  & `r PR3_res$S3`  & 1/3 S$_3$       \\ \hline
\end{tabular}
\end{table}


#### Příklad 4  
```{r, echo = FALSE}
PR4_res <- list()
PR4_res$cv <- (PR4$Q1 * PR4$c1 + PR4$Q2 + PR4$c2) / (PR4$Q1 + PR4$Q2)
```
Koncentrace po soutoku je rovna $c$ = `r round(PR4_res$cv, 3)` mg$\cdot$ l$^{-1}$. 

#### Příklad 5
```{r, echo = FALSE}
PR5_res <- list()
PR5_res$O <- round(PR5$b + 2 * PR5$h * sqrt(1.5 * 1.5 + 1), 2)
PR5_res$S <- round(PR5$h * (PR5$b + 1.5 * PR5$b), 2)
PR5_res$R <- round(PR5_res$S / PR5_res$O, 2)
PR5_res$C <- round((1 / PR5$n) * (PR5_res$R)^(1 / 6), 2)
PR5_res$v <- round(PR5_res$C * sqrt(PR5_res$R * 0.02), 2)
PR5_res$Q <- round(PR5_res$S * PR5_res$v, 2)
```

\begin{table}[h!]
\centering
\begin{tabular}{|l|c|}
\hline
Jednotka                & Výsledek            \\ \hline
O (m)                   & `r PR5_res$O`       \\ \hline
S (m$^{2}$)             & `r PR5_res$S`       \\ \hline
R (-)                   & `r PR5_res$R`       \\ \hline
C (m$^{0.5}.$s$^{-1}$)  & `r PR5_res$C`       \\ \hline
v (m$\cdot$ s $^{-1}$)  &`r PR5_res$v`        \\ \hline
Q (m$^{3}$.s $^{-1}$)   & `r PR5_res$Q`       \\ \hline
\end{tabular}
\end{table}
