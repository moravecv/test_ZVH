...
Vypočítejte bilanci vodní nádrže o objemu `r paste(PR5$V_text,"000", sep = " ")` m$^{3}$ s plochou vodní hladiny `r paste(PR5$A,"000", sep = " ")` m$^{2}$. Když byla na začátku roku `r PR5$stav`. Uvažujte roční výpar `r PR5$ET` mm.
...
\hline
Profil                 & 1              & 2               & 3               & 4               \\ \hline
v (m$\cdot$ s $^{-1}$) & `r PR3$v1`     & \textbf{`r PR3_res$v2`}  & `r PR3$v3`      & \textbf{`r PR3_res$v4`}  \\ \hline
Q (l$\cdot$ s $^{-1}$) & \textbf{`r PR3_res$Q1`} & \textbf{`r PR3_res$Q2`}  & 2Q$_{2}$        & Q$_2$/4         \\ \hline
D (cm)                 & `r PR3$D1`     & `r PR3$D2`      & \textbf{`r PR3_res$D3`}  & \textbf{`r PR3_res$D4`}  \\ \hline
S (cm$^{2}$)           & \textbf{`r PR3_res$S1`} & \textbf{`r PR3_res$S2`}  & \textbf{`r PR3_res$S3`}  & 1/3 S$_3$       \\ \hline
\end{tabular}
\end{table}
